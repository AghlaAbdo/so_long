...
/*   Updated: 2024/01/22 17:47:07 by aaghla           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_plr_frames(t_data *data, int der)
{
	...
	if (der == 1)
	{
		data->p_img = mlx_xpm_file_to_image(data->mlx_ptr, P_L_F_1, &w, &h);
	}
	else if (der == 0)
	{
		data->p_img = mlx_xpm_file_to_image(data->mlx_ptr, P_R_F_1, &w, &h);
	}
	if (!data->p_img)
	{
